...
class Environment:
    debug = True
    ticks = 0
    animals = {Species.Fox: [], Species.Rabbit: []}

    plants = {Species.Carrot: []}
    ...
    def distanceToAgent(self, agent1, agent2):
        return math.sqrt(
            (agent2.coord.x - agent1.coord.x) ** 2
            + (agent2.coord.y - agent1.coord.y) ** 2
        )
    ...
    def printStats(self, species):
        ...
        print(
            "Speed Mean = %.2f, Stdev = %.2f"
            % (statistics.mean(speed_list), speed_stdev)
        )
        print(
            "Sense Mean = %.2f, Stdev = %.2f"
            % (statistics.mean(sense_list), sense_stdev)
        )
        ...
        for spec in self.plants.keys():
            for plant in self.plants[spec]:
                ...
                if plant_dist < animal_base.genes.sense:
                    if (
                        surrounding_res.closest_plant is None
                        or plant_dist < surrounding_res.closest_plant_dist
                    ):
