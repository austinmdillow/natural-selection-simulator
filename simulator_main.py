...
def draw_actors(surface, selected_animal):
    ...
    for animal in environment.animals[Species.Fox]:
        ...
        if animal == selected_animal:
            animal_color = BLUE
        pygame.draw.rect(
            surface, animal_color, (x, y, 1 * animal.genes.size, 1 * animal.genes.size)
        )
        ...
        if animal.detected == "plant":
            ...
            circle_color = WHITE
        pygame.draw.circle(
            surface, circle_color, (int(x), int(y)), int(animal.genes.sense), 1
        )
...
def draw_stats(surface, font):
    text = font.render(
        "Alive: "
        + str(len(environment.animals[Species.Rabbit]))
        + ", "
        + str(len(environment.animals[Species.Fox])),
        True,
        GREEN,
        WHITE,
    )
    text2 = font.render(
        "FPS: " + str(FPS) + " " + str(int(fpsClock.get_fps())), True, GREEN, WHITE
    )
